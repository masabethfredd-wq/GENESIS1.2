...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // ── Counter animation (Senior Professional) ────────
        ...
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    ...
                    const duration = 2500; // 2.5 seconds for a dynamic but smooth pace
                    
                    // Add a slight delay before starting to sync with reveal-up
                    setTimeout(() => {
                        ...
                        function updateCounter(timestamp) {
                            ...
                            // Using Math.round prevents the "hanging" pause on the second-to-last number
                            let currentValue = Math.round(eased * target);
                            
                            // Ensure we don't overshoot the target due to rounding
                        ...
                        requestAnimationFrame(updateCounter);
                    }, 600); // Wait for the card to fade up
                    
                    observer.unobserve(el);
                }
            });
        }, { threshold: 0.8 }); // Wait until more of the element is visible
    ...
    function initTypewriterEffect() {
        ...
        verseEl.textContent = ''; // Clear it initially
        verseEl.style.opacity = 1; // Ensure container is visible
        
        const observer = new IntersectionObserver((entries) => {
            entries.forEach(entry => {
                if (entry.isIntersecting) {
                    let i = 0;
                    const speed = 55; // Slower than original, but faster than 75
                    
                    // Add cursor
                    verseEl.innerHTML = '<span class="typewriter-cursor">|</span>';
                    
                    function typeWriter() {
                        if (i < text.length) {
                            // Insert character before cursor
                            verseEl.innerHTML = text.substring(0, i + 1) + '<span class="typewriter-cursor">|</span>';
                            i++;
    ...
    function initIntelligenceNetwork() {
        ...
        function activateNode(node) {
            ...
            // Vanguard Positioning Logic: Move towards center from node
            const network = document.querySelector('.neural-network');
            const netRect = network.getBoundingClientRect();
            const nodeRect = node.getBoundingClientRect();
            
            const x = (nodeRect.left + nodeRect.width / 2) - netRect.left;
            const y = (nodeRect.top + nodeRect.height / 2) - netRect.top;
            
            const centerX = netRect.width / 2;
            const centerY = netRect.height / 2;
            
            const dx = x - centerX;
            const dy = y - centerY;
            const dist = Math.sqrt(dx * dx + dy * dy);
            
            // The card remains dead center.
            intelligenceCard.style.setProperty('left', '50%', 'important');
            intelligenceCard.style.setProperty('top', '50%', 'important');
            intelligenceCard.style.setProperty('transform', 'translate(-50%, -50%) scale(1)', 'important');
            intelligenceCard.style.setProperty('right', 'auto', 'important');
            intelligenceCard.style.setProperty('bottom', 'auto', 'important');
